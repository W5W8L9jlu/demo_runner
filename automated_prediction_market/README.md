...
This repository is a standalone extraction of the upstream `automated-prediction-market` demo from `reactive-smart-contract-demos`.

## Repository Setup

Initialize the pinned `reactive-lib` submodule before building or deploying:
...
**Origin/Destination Chain Contract**: [AutomatedPredictionMarketCallback](./src/AutomatedPredictionMarketCallback.sol) manages the full lifecycle of prediction markets on Ethereum Sepolia: market creation, share purchases with referral rewards, resolution proposals, and multisig-gated voting. When sufficient votes are cast, the contract finalises the outcome on-chain and emits a `PredictionResolved` event. Winnings are then distributed in batches via a callback from the Reactive Network by the `distributeWinnings` function, which is protected by `authorizedSenderOnly`.

**Reactive Contract**: [AutomatedPredictionReactive](./src/AutomatedPredictionReactive.sol) subscribes to `PredictionResolved` events emitted by the callback contract on Sepolia. When the event fires, the `react` function decodes the prediction ID and emits a `Callback` event targeting `distributeWinnings` on Sepolia, closing the cross-chain loop automatically.
...
Deploy `AutomatedPredictionMarketCallback` on Ethereum Sepolia, passing the Sepolia callback proxy address. Assign the `Deployed to` address to `CALLBACK_ADDR`.

```bash
forge create --broadcast --rpc-url $DESTINATION_RPC --private-key $DESTINATION_PRIVATE_KEY src/AutomatedPredictionMarketCallback.sol:AutomatedPredictionMarketCallback --value 1ether --constructor-args $DESTINATION_CALLBACK_PROXY_ADDR
```
...
```bash
forge create --broadcast --rpc-url $REACTIVE_RPC --private-key $REACTIVE_PRIVATE_KEY src/AutomatedPredictionReactive.sol:AutomatedPredictionReactive --value 1ether --constructor-args $CALLBACK_ADDR
```
